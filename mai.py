...
class LeafDiseaseDetector:
    ...
    def create_analysis_prompt(self) -> str:
        ...
        SEVERITY — look at the leaf image carefully and choose based on what you actually see:

        "severe" — use this when ANY of these are visible:
          • More than 30% of the visible leaf area has spots, lesions, discolouration, or damage
          • Brown, black, or dead (necrotic) patches are clearly present
          • The leaf edges or tips are burnt, dried, or curling/wilting
          • Large merged spots that cover significant portions of the leaf
          • The leaf looks like it is dying or has already partially died
          • Holes, tears, or skeletal damage from pests covering a large area
          IMPORTANT: When you are unsure between "moderate" and "severe", always choose "severe".
          A farmer losing a crop is worse than over-cautioning.

        "moderate" — use this when:
          • 10% to 30% of the leaf is affected
          • Several distinct spots or lesions visible but most of the leaf is still green and alive
          • Disease is clearly present and progressing but the leaf is mostly intact

        "mild" — use this ONLY when:
          • Less than 10% of the leaf is affected
          • Only 1 to 3 small spots or marks, rest of leaf looks completely healthy
          • Very early stage, easy to miss at first glance

        "none" — healthy leaf with no disease at all

        AFFECTED AREA — estimate what percentage of the total leaf surface is visibly damaged.
        Be honest — if it looks like 40% is damaged, say 40, not 20.

        SPREAD RISK:
          "high"   → fungal or bacterial + severity is severe
          "medium" → fungal or bacterial with moderate, OR any viral disease
          "low"    → mild severity, pest damage, or nutrient deficiency
          "none"   → healthy leaf

        For VALID LEAF images, return this format:
        {
            "disease_detected": true/false,
            "plant_name": "common name of the plant/crop (e.g. Spinach, Tomato, Rice, Mango, Cotton)",
            ...
            "spread_risk": "low/medium/high/none"
        }"""
    ...
    def _parse_response(self, response_content: str) -> DiseaseAnalysisResult:
        ...
        try:
            ...
            return DiseaseAnalysisResult(
                ...
                affected_area_percent=int(disease_data.get('affected_area_percent', 0)),
                spread_risk=disease_data.get('spread_risk', 'none'),
                plant_name=disease_data.get('plant_name', 'Unknown')
            )
